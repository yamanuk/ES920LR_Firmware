...
void SX1276LoRaInit( void )
{
    ...
    SX1276ReadBuffer( 0x01, SX1276Regs + 1, 0x70 - 1 );

    SX1276LR->RegLna = 0x20;

    SX1276WriteBuffer( 0x01, SX1276Regs + 1, 0x70 - 1 );
    ...
    SX1276LoRaSetLowDatarateOptimize( 0x01u );

    if( LoRaSettings.RFFrequency > 860000000 )
    {
        SX1276LoRaSetPAOutput( 0x00 );
        SX1276LoRaSetPa20dBm( 0x00u );
        LoRaSettings.Power = 14;
        SX1276LoRaSetRFPower( LoRaSettings.Power );
    }
    else
    {
        SX1276LoRaSetPAOutput( 0x80 );
        SX1276LoRaSetPa20dBm( 0x01u );
        LoRaSettings.Power = 20;
        SX1276LoRaSetRFPower( LoRaSettings.Power );
    }

    SX1276LoRaSetOpMode( 0x01 );
}
...
void SX1276LoRaSetDefaults( void )
{
    ...
    SX1276Read( 0x42, &SX1276LR->RegVersion );
}

void SX1276LoRaSetOpMode( uint8_t opMode )
{
    static uint8_t opModePrev = 0x01;

    opModePrev = SX1276LR->RegOpMode & ~0xF8;

    if( opMode != opModePrev )
    {
        SX1276LR->RegOpMode = ( SX1276LR->RegOpMode & 0xF8 ) | opMode;

        SX1276Write( 0x01, SX1276LR->RegOpMode );
    }
}

uint8_t SX1276LoRaGetOpMode( void )
{
    SX1276Read( 0x01, &SX1276LR->RegOpMode );
    return SX1276LR->RegOpMode & ~0xF8;
}

uint8_t SX1276LoRaReadRxGain( void )
{
    SX1276Read( 0x0C, &SX1276LR->RegLna );
...
double SX1276LoRaReadRssi( void )
{
    ...
    SX1276Read( 0x1B, &SX1276LR->RegRssiValue );

    if( LoRaSettings.RFFrequency < 860000000 )  
    {
        return -164.0 + ( double )SX1276LR->RegRssiValue;
    }
    else
    {
        return -157.0 + ( double )SX1276LR->RegRssiValue;
    }
}
...
uint32_t SX1276LoRaProcess( void )
{
    ...
    case RFLR_STATE_RX_INIT:
        ...
        SX1276LR->RegIrqFlagsMask = 0x80 |
                                    ...
                                    0x01;
        SX1276Write( 0x11, SX1276LR->RegIrqFlagsMask );

        if( LoRaSettings.FreqHopOn == 0x01u )
        {
            SX1276LR->RegHopPeriod = LoRaSettings.HopPeriod;

            SX1276Read( 0x1C, &SX1276LR->RegHopChannel );
            SX1276LoRaSetRFFrequency( HoppingFrequencies[SX1276LR->RegHopChannel & 0x3F] );
        }
        else
        {
        ...
        SX1276LR->RegDioMapping1 = 0x00 | 0x00 | 0x00 | 0x00;
                                    
        SX1276LR->RegDioMapping2 = 0x00 | 0x10;
        SX1276WriteBuffer( 0x40, &SX1276LR->RegDioMapping1, 2 );

        if( LoRaSettings.RxSingleOn == 0x01u ) 
        {
            SX1276LoRaSetOpMode( 0x06 );
        }
        else 
        {
            SX1276LR->RegFifoAddrPtr = SX1276LR->RegFifoRxBaseAddr;
            SX1276Write( 0x0D, SX1276LR->RegFifoAddrPtr );

            SX1276LoRaSetOpMode( 0x05 );
        }

        memset( RFBuffer, 0, ( size_t )256 );

        PacketTimeout = LoRaSettings.RxPacketTimeout;
        RxTimeoutTimer = ( TickCounter );
    ...
    case RFLR_STATE_RX_RUNNING:
        ...
        if( LoRaSettings.RxSingleOn == 0x01u ) 
        {
            if( ( ( TickCounter ) - RxTimeoutTimer ) > PacketTimeout )
            {
    ...
    case RFLR_STATE_RX_DONE:
        ...
        SX1276Read( 0x12, &SX1276LR->RegIrqFlags );
        if( ( SX1276LR->RegIrqFlags & 0x20 ) == 0x20 )
        {
            
            SX1276Write( 0x12, 0x20  );
        ...
        SX1276Read( 0x1A, &SX1276LR->RegPktRssiValue );

        if( LoRaSettings.RFFrequency < 860000000 )  
        {
            if( RxPacketSnrEstimate < 0 )
            {
                RxPacketRssiValue = -164.0 + ( ( double )SX1276LR->RegPktRssiValue ) + RxPacketSnrEstimate;
            }
            else
            {
                RxPacketRssiValue = -164.0 + ( 1.0666 * ( ( double )SX1276LR->RegPktRssiValue ) );
            }
        }
        else                                        
        {
            if( RxPacketSnrEstimate < 0 )
            {
                RxPacketRssiValue = -157.0 + ( ( double )SX1276LR->RegPktRssiValue ) + RxPacketSnrEstimate;
            }
            else
            {
                RxPacketRssiValue = -157.0 + ( 1.0666 * ( ( double )SX1276LR->RegPktRssiValue ) );
            }
        }
        ...
        if( LoRaSettings.RxSingleOn == 0x01u ) 
        {
            ...
            if( LoRaSettings.ImplicitHeaderOn == 0x01u )
            {
            ...
            {
                SX1276Read( 0x13, &SX1276LR->RegNbRxBytes );
        ...
        {
            SX1276Read( 0x10, &SX1276LR->RegFifoRxCurrentAddr );
    ...
    case RFLR_STATE_TX_INIT:
        ...
        SX1276Write( 0x0E, SX1276LR->RegFifoTxBaseAddr );
        ...
        SX1276LR->RegDioMapping1 = 0x40 | 0x00 | 0x00 | 0x01;
                                        
        SX1276LR->RegDioMapping2 = 0x40 | 0x10;
        SX1276WriteBuffer( 0x40, &SX1276LR->RegDioMapping1, 2 );
...
uint32_t SX1276LoRaCcaProcess( void )
{
    ...
    while( 1 )
    {
        ...
        ((((SIM_MemMapPtr)0x40047000u))->SRVCOP) = 0x55;
        ((((SIM_MemMapPtr)0x40047000u))->SRVCOP) = 0xAA;
    }
