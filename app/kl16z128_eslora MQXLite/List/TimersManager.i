...
static tmrTimerTableEntry_t maTmrTimerTable[( 8 + 10 )];
...
static tmrStatus_t maTmrTimerStatusTable[( 8 + 10 )];
...
osThreadLink_t osThreadLink_TMR_Thread[1] = {0}; uint8_t osThread_TMR_Thread_stack[(700)*(1)]; uint8_t osThread_TMR_Thread_signal[36 * (1)]; osThreadDef_t os_thread_def_TMR_Thread = { (TMR_Thread), (osPriorityRealtime), (1), (700), osThread_TMR_Thread_stack, osThread_TMR_Thread_signal, osThreadLink_TMR_Thread, {"TMR_Thread"}};
...
)
{
  ...
  ( (((RTC_MemMapPtr)0x4003D000U)->IER) &= (uint32_t)(~(uint32_t)0x4u) );
  if( gRTCAlarmCallback != (( void * )( 0x0UL )) )
  {
...
{
    gTimerHandle = StackTimer_Init((( void * )( 0x0UL )));
    mCounterFreqHz = (uint32_t)((StackTimer_GetInputFrequency(gTimerHandle)));  
    ...
    mTimerThreadId = osThreadCreate(&os_thread_def_TMR_Thread, (( void * )( 0x0UL )));

    if( mTimerThreadId == (( void * )( 0x0UL )) )
    {
...
{
  (void)UserDataPtr;
  osSignalSet( mTimerThreadId, (1<<0) );
}
...
{
    return TMR_GetTimerStatus(timerID) == 0x40;
}
...
)
{
  ...
  saveInt = 0; osInterruptDisable();;

  (currentTime) = StackTimer_GetCounterValue(gTimerHandle);;
  ...
  {
    (void)saveInt; osInterruptEnable();;
    return 1;
  }
...
)
{
    ...
    (maTmrTimerTable[timerID]. timestamp) = StackTimer_GetCounterValue(gTimerHandle);;
...
{
    TMR_StartTimer(timerId, timerType | 0x10, timeIn, pfTmrCallBack, param);
}
...
{
    TMR_StartTimer(timerId, ( 0x04 ), ( (tmrTimeInMilliseconds_t) (( (tmrTimeInMilliseconds_t) (( (tmrTimeInMilliseconds_t) (timeInMinutes) ) * 1000) ) * 60) ), pfTmrCallBack, param);
}
...
{
    TMR_StartTimer(timerId, ( 0x08 ), ( (tmrTimeInMilliseconds_t) (( (tmrTimeInMilliseconds_t) (timeInSeconds) ) * 1000) ), pfTmrCallBack, argument);
}
...
{
    TMR_StartTimer(timerID, 0x02, timeInMilliseconds, pfTimerCallBack, param);
}
...
{
    TMR_StartTimer(timerID, 0x01, timeInMilliseconds, pfTimerCallBack, param);
}
...
)
{
    ...
    while(1)
    {
        ...
        (currentTimeInTicks) = StackTimer_GetCounterValue(gTimerHandle);;
        ...
        (ticksdiff) = StackTimer_GetCounterValue(gTimerHandle);;
        ...
        if(numberOfActiveTimers || numberOfLowPowerActiveTimers)  
        {
          ...
          StackTimer_SetOffsetTicks(gTimerHandle, 0, nextInterruptTime);
        }
        (void)saveInt; osInterruptEnable();;

        if (!numberOfActiveTimers && !numberOfLowPowerActiveTimers)
        {
            StackTimer_Disable(gTimerHandle);
            timerHardwareIsRunning = 0;
        }
        else if (!timerHardwareIsRunning)
        {
            StackTimer_Enable(gTimerHandle);
...
)
{
    ...
    StackTimer_Enable(gTimerHandle);
    (previousTimeInTicks) = StackTimer_GetCounterValue(gTimerHandle);;
...
void TMR_RTCInit()
{
  ...
  gRTCHandle = RealTimeClock_Init((( void * )( 0x0UL )), 1);
  ...
  gRTCAlarmCallback = (( void * )( 0x0UL ));
  gRTCAlarmCallbackParam = (( void * )( 0x0UL ));

  ( (((RTC_MemMapPtr)0x4003D000U)->IER) &= (uint32_t)(~(uint32_t)0x4u) );
  ( (((RTC_MemMapPtr)0x4003D000U)->TSR) = (uint32_t)(0x00 + 0x01) );
  ( (((RTC_MemMapPtr)0x4003D000U)->TPR) = (uint32_t)(0x00) );

  if(RealTimeClock_Enable(gRTCHandle) != 0x00U)
  {
...
uint64_t TMR_RTCGetTimestamp()
{
  ...
  prescaler0 = ( (((RTC_MemMapPtr)0x4003D000U)->TPR) );
  seconds1 = ( (((RTC_MemMapPtr)0x4003D000U)->TSR) );
  prescaler1 = ( (((RTC_MemMapPtr)0x4003D000U)->TPR) );
  seconds2 = ( (((RTC_MemMapPtr)0x4003D000U)->TSR) );
  prescaler2 = ( (((RTC_MemMapPtr)0x4003D000U)->TPR) );
...
void TMR_RTCSetTime(uint64_t microseconds)
{
  ...
  initialAlarm = ( (((RTC_MemMapPtr)0x4003D000U)->TAR) ) + (initialAlarm/1000000L);
  ...
  {
    ( (((RTC_MemMapPtr)0x4003D000U)->TAR) = (uint32_t)(initialAlarm - (microseconds/1000000L)) );
  }
...
void TMR_RTCSetAlarm(uint64_t seconds, pfTmrCallBack_t callback, void *param)
{
  ...
  ( (((RTC_MemMapPtr)0x4003D000U)->TAR) = (uint32_t)((uint32_t)seconds) );
...
void TMR_RTCSetAlarmRelative(uint32_t seconds, pfTmrCallBack_t callback, void *param)
{
  ...
  RealTimeClock_Disable(gRTCHandle);
  rtcSeconds = ( (((RTC_MemMapPtr)0x4003D000U)->TSR) );
  rtcPrescaler = ( (((RTC_MemMapPtr)0x4003D000U)->TPR) );
  ( (((RTC_MemMapPtr)0x4003D000U)->TPR) = (uint32_t)(0x00) );
  
 
  ( (((RTC_MemMapPtr)0x4003D000U)->TSR) = (uint32_t)(rtcSeconds) );
  RealTimeClock_Enable(gRTCHandle);
  rtcPrescaler &= 0x7fff;
  ...
  gRTCAlarmCallback = callback;
  gRTCAlarmCallbackParam = param;
  ( (((RTC_MemMapPtr)0x4003D000U)->TAR) = (uint32_t)(seconds + rtcSeconds - 1) );
  ( (((RTC_MemMapPtr)0x4003D000U)->IER) |= 0x4u );
