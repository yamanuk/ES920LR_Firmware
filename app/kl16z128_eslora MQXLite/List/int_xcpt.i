...
{  
    ...
    if ( kernel_data->IN_ISR > 1 ) {
        ...
        if (isr_frame_ptr == ((pointer)0)) {
             
            _mqx_fatal_error(((0x0000)|0x20));
        }
        ...
        if (exception_handler) {
            (*exception_handler)(isr_vector, (_mqx_uint)parameter, _int_get_isr_data(isr_vector) , exception_frame_ptr);
        }

        } else {
             
            _mqx_fatal_error(((0x0000)|0x14));
        }
        ...
        if (td_ptr->EXCEPTION_HANDLER_PTR != ((pointer)0) ) {
            (*td_ptr->EXCEPTION_HANDLER_PTR)((_mqx_uint)parameter,
            td_ptr->STACK_PTR);
        } else {
             
            _task_abort(((_task_id)0));
        }
   }
}
