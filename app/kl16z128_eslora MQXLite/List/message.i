...
static uint8_t           gau8RxDataBuffer[( 128 )];
...
void DoOperation( void )
{
    ...
    if( 2 == mTermParam.Node )
    {
        if( 2 == mTermParam.Sleep )
        {
             
            llwu_configure( 0x0000, 2, 0x01 );
        }
        else if( 3 == mTermParam.Sleep )
        {
             
            llwu_configure( 0x0040, 2, 0x00 );
        }
    }
    ...
    if( 1 == mTermParam.Ack )
    {
        txConfig.autoAck = ((boolean)1);
        txConfig.retryCountAckFail = mTermParam.Retry;
    }
    else
    {
        txConfig.autoAck = ((boolean)0);
        txConfig.retryCountAckFail = 0;
    }
    txConfig.ccaBeforeTx       = ((boolean)1);
    ...
    TMR_StartIntervalTimer( IntervalTmr, ( 100 ), IntervalTimerCallBack, ((pointer)0) );
    ...
    if( 0 != mTermParam.SendTime )
    {
        ...
        {
            TMR_StartSingleShotTimer( SendTimeTmr, mTermParam.SendTime * 1000, SendTimeTimerCallBack, ((pointer)0) );
        }
    }
...
static void ProcessEvent( void )
{
    ...
    ev = osSignalWait( 0, 0xFFFFFFFF );

     
    if( (1<<0) & ev.value.signals )
    {
    ...
    if( (1<<1) & ev.value.signals )
    {
        ...
        if( (0 != mTermParam.SendTime) && (1 == mTermParam.Sleep) )
        {
            TMR_StartSingleShotTimer( SendTimeTmr, mTermParam.SendTime * 1000, SendTimeTimerCallBack, ((pointer)0) );
        }
    }
...
static smacErrors_t SendPacketProcess( uint8_t data_length )
{
    ...
    if( ((( 50 ) < data_length) && (1 == mTermParam.TransMode)) ||
        (((( 50 ) + 8) < data_length) && (2 == mTermParam.TransMode)) )
    {
    ...
    if( (data_length <= 8) && (2 == mTermParam.TransMode) )
    {
    ...
    if( 1 == mTermParam.TransMode )
    {
        ...
        gAppTxPacket->smacHeader.data_type   = 0x01;
    ...
    {
        gAppTxPacket->smacHeader.totalLength = data_length - 8;
        gAppTxPacket->smacHeader.data_type   = 0x01;

         
        SMAC_SetPanID( (( ('a' <= SendDataBuffer[0]) ? (SendDataBuffer[0] - 'a' + 10) : (('A' <= SendDataBuffer[0]) ? (SendDataBuffer[0] - 'A' + 10) : (SendDataBuffer[0] - '0')) ) << 12) | (( ('a' <= SendDataBuffer[1]) ? (SendDataBuffer[1] - 'a' + 10) : (('A' <= SendDataBuffer[1]) ? (SendDataBuffer[1] - 'A' + 10) : (SendDataBuffer[1] - '0')) ) << 8) |
                       (( ('a' <= SendDataBuffer[2]) ? (SendDataBuffer[2] - 'a' + 10) : (('A' <= SendDataBuffer[2]) ? (SendDataBuffer[2] - 'A' + 10) : (SendDataBuffer[2] - '0')) ) << 4)  |  ( ('a' <= SendDataBuffer[3]) ? (SendDataBuffer[3] - 'a' + 10) : (('A' <= SendDataBuffer[3]) ? (SendDataBuffer[3] - 'A' + 10) : (SendDataBuffer[3] - '0')) ) );
        SMAC_SetSrcAddress( mTermParam.SrcId );
        SMAC_FillHeader( &(gAppTxPacket->smacHeader),
                         (( ('a' <= SendDataBuffer[4]) ? (SendDataBuffer[4] - 'a' + 10) : (('A' <= SendDataBuffer[4]) ? (SendDataBuffer[4] - 'A' + 10) : (SendDataBuffer[4] - '0')) ) << 12) | (( ('a' <= SendDataBuffer[5]) ? (SendDataBuffer[5] - 'a' + 10) : (('A' <= SendDataBuffer[5]) ? (SendDataBuffer[5] - 'A' + 10) : (SendDataBuffer[5] - '0')) ) << 8) |
                         (( ('a' <= SendDataBuffer[6]) ? (SendDataBuffer[6] - 'a' + 10) : (('A' <= SendDataBuffer[6]) ? (SendDataBuffer[6] - 'A' + 10) : (SendDataBuffer[6] - '0')) ) << 4)  |  ( ('a' <= SendDataBuffer[7]) ? (SendDataBuffer[7] - 'a' + 10) : (('A' <= SendDataBuffer[7]) ? (SendDataBuffer[7] - 'A' + 10) : (SendDataBuffer[7] - '0')) ) );

        err = SendPacket( (SendDataBuffer + 8), (data_length - 8) );
    }
...
smacErrors_t SendPacket( uint8_t* pData, uint8_t DataLen )
{
    ...
    if( ((boolean)1) == txConfig.autoAck )
    {
        ackTime = 50 + (uint32_t)ceil((symbolTime * 2.0) + (symbolTime * 12.25) +    
                  (symbolTime * (8+ceil((80-4.0*mTermParam.Sf+44)/(4.0*(mTermParam.Sf-2)))*5)));

        dataTime = (uint32_t)ceil((symbolTime * 12.25) +                            
                   (symbolTime * (8+ceil((8*(12+txLen)-4.0*mTermParam.Sf+44)/(4.0*(mTermParam.Sf-2)))*5)));

        sendTmo = 100 + (dataTime + ackTime) * (txConfig.retryCountAckFail + 1);
    }
    else
    {
        dataTime = (uint32_t)ceil((symbolTime * 12.25) +                            
    ...
    TMR_StartSingleShotTimer( WaitSendTmr, sendTmo, WaitSendTimerCallBack, ((pointer)0) );
...
static void ReceiveData( rxPacket_t* pMsgIn, int16_t rssi )
{
    ...
    if( (1 == mTermParam.Rssi) && (1 == mTermParam.RcvId) )
    {
        len = pMsgIn->smacHeader.totalLength + 12;
    }
    else if( (1 == mTermParam.Rssi) && (2 == mTermParam.RcvId) )
    {
        len = pMsgIn->smacHeader.totalLength + 4;
    }
    else if( (2 == mTermParam.Rssi) && (1 == mTermParam.RcvId) )
    {
...
static void SendDoneProcess( void )
{
    ...
    if( ((boolean)1) == bCCAFailed )
    {
        Terminal_Print( "Send Failed\r\n" );
        bCCAFailed = ((boolean)0);

        if( !(gSendingType & ( 0x80 )) )
        {
        	if( 1 == mTermParam.Format )
        	{
    ...
    else if( ((boolean)1) == bAckFailed )
    {
        Terminal_Print( "Ack Timeout\r\n" );
        bAckFailed = ((boolean)0);
...
static void SleepProcess( void )
{
    ...
    if( (2 == mTermParam.Node) && (((boolean)1) == gEnterIdle) )
    {
        if( 2 == mTermParam.Sleep )
        {
        ...
        if( (2 == mTermParam.Sleep) || (3 == mTermParam.Sleep) )
        {
            LED_TurnOffLed( 0x03 );
        ...
        if( 2 == mTermParam.Sleep )
        {
            ...
            ((((GPIO_MemMapPtr)0x400FF0C0u))->PCOR) |= (uint32_t)(1<<4);
        ...
        if( (2 == mTermParam.Sleep) || (3 == mTermParam.Sleep) )
        {
            ...
            ((((GPIO_MemMapPtr)0x400FF0C0u))->PSOR) |= (uint32_t)(1<<4);
...
{
    bCCAFailed = ((boolean)1);
    osSignalSet( myAppId, (1<<1) );
}
...
{
    if( ((((PORT_MemMapPtr)0x4004B000u))->PCR[1]) & 0x1000000u )
    {
        ((((PORT_MemMapPtr)0x4004B000u))->PCR[1]) |= 0x1000000u ;

        if( 3 == mTermParam.Sleep )
        {
            gEnterIdle  = ((boolean)1);

            osSignalSet( myAppId, (1<<7) );
        }
    }
}
