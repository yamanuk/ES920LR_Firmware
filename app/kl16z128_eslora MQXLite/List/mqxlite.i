...
const char * const _mqx_path                = "C:/Freescale/PExDrv v10.3/eclipse/ProcessorExpert/lib/mqxlite/V1.0.1/";
 
const char * const _mqx_generic_revision    = "GEN_REVISION";
const char * const _mqx_psp_revision        = "PSP_REVISION";
const char * const _mqx_copyright           = "(c) 2011 Freescale Semiconductor. All rights reserved.";
const char * const _mqx_date                = "May 14 2020" " at " "16:51:00";

 
const uint_32      _mqxlite_version_number  = 0x01000001;
const char * const _mqxlite_version         = "V1.0.1";


 
 struct   kernel_data_struct * _mqx_kernel_data = (pointer)-1;
...
{  
    ...
    _mqx_kernel_data = (struct kernel_data_struct *)(kernel_data);
    ...
    kernel_data->INTERRUPT_STACK_PTR = ((pointer)((_mem_size)(((uchar_ptr)(stack_ptr) + result - 1)) & (~((0x10)-1))));
    ...
    _mqx_set_cpu_type(((((4) & (0x0f)) << (12)) | ((0x00 & (0xff)) << (4)) | ((0x00 & (0x0f)) << (0))));

    result = _psp_int_init((INT_SysTick), (INT_PORTC_PORTD));
    if (result != (0)) {
        return(result);  
    }
    ...
    {
        SystemTimer1_Init(((pointer)0));

        SystemTimer1_GetPeriodTicks(((pointer)0), (uint32_t *)&kernel_data->HW_TICKS_PER_TICK);

        kernel_data->TICKS_PER_SECOND = SystemTimer1_GetTicsPerSecond(((pointer)0));

        _time_set_hwtick_function((MQX_GET_HWTICKS_FPTR)SystemTimer1_GetCounterValue, (((pointer)0)));
    }
...
{  
    ...
    if ( setjmp(_mqx_exit_jump_buffer_internal) ) {
     ...
     
    {
        ...
        while (template_ptr->TASK_TEMPLATE_INDEX) {
            if (template_ptr->TASK_ATTRIBUTES & (0x01)) {
...
{  
    ...
    longjmp(_mqx_exit_jump_buffer_internal,1);
}  
