    ...
    )
{
    ...
    kernel_data->DISABLE_SR = (((temp) << (8 - (1))) & ((0xff << (8 - (1))) & 0xff));

     
    for (i = 0; i < sizeof(((NVIC_MemMapPtr)0xE000E100u)->IP) / sizeof(((NVIC_MemMapPtr)0xE000E100u)->IP[0]); i++)
        ((NVIC_MemMapPtr)0xE000E100u)->IP[i] = ((((1 << (1)) - 2) << (8 - (1))) & ((0xff << (8 - (1))) & 0xff));


     
    {
        uint32_t * icer_ptr = (uint32_t *)&((NVIC_MemMapPtr)0xE000E100u)->ICER;

        for (i = 0; i < sizeof(((NVIC_MemMapPtr)0xE000E100u)->ICER) / sizeof(uint32_t); i++)    {
...
{  
   ...
   uint_32                n;
    ...
    kernel_data = _mqx_kernel_data;
    kernel_data->READY_Q_LIST = (READY_Q_STRUCT_PTR) ((pointer)0);
    ...
    while (n--) {
        ...
        if (n + kernel_data->INIT.MQX_HARDWARE_INTERRUPT_LEVEL_MAX < ((1 << (1)) - 1))
            q_ptr->ENABLE_SR   = (((n + kernel_data->INIT . MQX_HARDWARE_INTERRUPT_LEVEL_MAX) << (8 - (1))) & ((0xff << (8 - (1))) & 0xff));
        else
            q_ptr->ENABLE_SR   = ((((1 << (1)) - 2) << (8 - (1))) & ((0xff << (8 - (1))) & 0xff));
