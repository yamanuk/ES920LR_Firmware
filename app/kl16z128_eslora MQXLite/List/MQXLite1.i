...
uint8_t main_task_stack[(sizeof(TD_STRUCT) + 768 + ((0x10)-1) + 1)];
...
READY_Q_STRUCT                         mqx_static_ready_queue[((21) + 1)];

 
INTERRUPT_TABLE_STRUCT                 mqx_static_isr_table[((INT_PORTC_PORTD) - (INT_SysTick) + 1)];


 
uint8_t mqx_interrupt_stack[512 + ((0x10)-1) + 1];
 
uint8_t mqx_idle_task_stack[sizeof(TD_STRUCT) + ((sizeof(PSP_STACK_START_STRUCT) + (64) + (0)) + 8) + ((0x10)-1) + 1];
 
uint8_t mqx_system_stack[(sizeof(PSP_STACK_START_STRUCT) + (64) + (0)) + ((0x10)-1) + 1];

 
const MQXLITE_INITIALIZATION_STRUCT  MQX_init_struct =
{
   1,
   (pointer)(&_mqx_kernel_data_struct),
   (pointer)((uchar_ptr)&_mqx_kernel_data_struct + sizeof(_mqx_kernel_data_struct)),
   1,
   sizeof(mqx_interrupt_stack),
   mqx_interrupt_stack,
   sizeof(mqx_idle_task_stack),
   mqx_idle_task_stack,
