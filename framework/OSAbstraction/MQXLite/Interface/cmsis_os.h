...
#define osNumberOfTimers       5       ///< maximum number of semaphores
#endif
...
#define osThreadStackDef(name, stacksize, instances) \
        uint8_t osThreadStackArray(name)[(stacksize)*(instances)];

#define osThreadSignalArray(name) osThread_##name##_signal
#define osThreadSignalDef(name, instances) \
        uint8_t osThreadSignalArray(name)[36 * (instances)];
...
#define osThreadDef(name, priority, instances, stacksz)  \
osThreadLink_t osThreadLink_##name##[instances] = {0}; \
osThreadStackDef(name, stacksz, instances) \
osThreadSignalDef(name, instances) \
osThreadDef_t os_thread_def_##name = { (name), \
                                       (priority), \
                                       (instances), \
                                       (stacksz), \
                                       osThreadStackArray(name), \
                                       osThreadSignalArray(name), \
                                       osThreadLink_##name##, \
                                       {#name}}
#endif
...
/// \param[in]     param         Pointer to the data to be passed as the first parameter to the
